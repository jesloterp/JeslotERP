# Messaging (`p14_messaging`)

**Package:** `p14_messaging`  
**Schema:** `messaging`  
...
Asynchronous job and queue control plane: enqueue, lease, retry, DLQ, fairness, and worker handlers.

## 2. Responsibilities

- Own the `messaging` persistence schema and the `p14_messaging` module boundary.
...
- Job APIs
- Queue policy, pause, and DLQ
- Admin handlers, batches, quotas, bridges
- Internal workers, leases, and sweeps
- Job ledger is persistence-backed
- Optional broker attach; fail-closed when not configured
...
- Runtime handlers invoke notification, integration, and licensing job types

### Recommended future dependency

- p17_scheduler as a producer of due work
- p15_notification dispatch
- p16_cache / p20_logging / p21_monitoring
...
- Scheduler
- Integration
- Licensing compile / lifecycle jobs

## 9. Events

Representative public event names observed in the platform (not an exhaustive private catalog):

- `messaging.job.enqueued`
- `messaging.job.succeeded`
- `messaging.job.dead_lettered`
- `messaging.queue.paused`
- `messaging.queue.depth_high`
...
Broker attach is optional at startup.
...
- Worker versus admin separation
- Handler allow-list
- RLS
...
| Module plugin | Yes |
| HTTP surface | Yes |
| Persistence schema `messaging` | Yes |
| Automated tests | Yes |
| Kernel | SoR-Live |
...
- Live Redis / Rabbit broker remains provider-pending
- Production worker fleet is not claimed
...
Long-running ERP work (posting, import, render, sync) must be jobs, not request threads.
