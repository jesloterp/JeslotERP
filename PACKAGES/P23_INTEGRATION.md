# Integration (`p23_integration`)

**Package:** `p23_integration`  
**Schema:** `integration`  
...
External connectivity control plane: connectors, connections, mappings, pipelines, outbound / inbound, webhooks, and DLQ.

## 2. Responsibilities

- Own the `integration` persistence schema and the `p23_integration` module boundary.
...
- Connector catalog
- Connection lifecycle, probe, rotate, circuit breaker, rate limits
- Mapping and pipeline versioning / publish
- Outbound delivery, attempts, and DLQ replay
- Inbound webhooks
- Partner profiles and reconcile
- Analytics and packs
- REST connector adapter with optional live mode
...
- Delivery jobs via p14_messaging
- Audit bridge

### Recommended future dependency

- p03_configuration secret refs
- p17_scheduler for poll connectors

Do not treat recommended lines as implemented wiring.

## 8. Consumers

- Future tax authority, bank, and marketplace adapters
- Notification / licensing job handlers
...
Live REST is explicitly gated; isolated tests disable live adapters.
...
| Module plugin | Yes |
| HTTP surface | Yes |
| Persistence schema `integration` | Yes |
| Automated tests | Yes |
| Kernel | SoR-Live |
...
- Non-REST enterprise connectors are not claimed as live products
- Some catalog / mapping / pipeline paths still have memory-double history
...
GST, banks, carriers, and marketplaces attach here — they must not be copied into every module.
